...
func TestStaticPodManaged(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedAnnotations: map[string]string{
				"workload.openshift.io/management":                 "",
				"io.openshift.workload.management.cpushares/image": "102",
			},
		},
		{
			pod: &v1.Pod{
	...
	for _, tc := range testCases {
		ModifyStaticPodForPinnedManagement(tc.pod)
		...
		for _, container := range tc.pod.Spec.Containers {
			...
			if _, exists := container.Resources.Requests[v1.ResourceName(ManagedCapacityLabel)]; !exists {
